...
# codeset = getCodeSet('symbols400')
# setUrlList(codeset)

def onlySetUrlList():
    # client = redis.Redis(host='10.11.9.7', port=6379, db=15, password='centos_data_123456', decode_responses=True)
    client = redis.Redis(host='r-2ze4635ef679bf64.redis.rds.aliyuncs.com', port=6379, db=0,password='rootdB2017',decode_responses=True)
    # client = redis.Redis(host='39.106.168.88', port=6379, db=0,password='rootdB2017',decode_responses=True)
    if client.exists('xueqiu_min_redis:start_urls'):
        return
    #codeset = client.smembers("symbols400")
    codeset = {'SP500','DJI30'}
    for code in codeset:
        # url = "https://xueqiu.com/stock/forchart/stocklist.json?symbol={}&period=1d&one_min=1&_={}".format(code, int(
        #     time.time() * 1000))
        url = "https://xueqiu.com/stock/forchart/stocklist.json?symbol={}&period=1d&one_min=1".format(code)
        client.rpush('xueqiu_min_redis:start_urls', url)
